...
__global__ void kernel_init(double* data, int nx, int ny, int nz, double u_0) {
...
int main(int argc, char* argv[]) {
	...
	while (f) {
		...
		if (ib + 1 < box[0]) {
			kernel_copy_send_LR<<<GRID_SIZE, BLOCK_SIZE>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], dim[0]-1);
			CSC(cudaGetLastError());
		...
		if (jb + 1 < box[1]) {
			kernel_copy_send_FB<<<GRID_SIZE, BLOCK_SIZE>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], dim[1]-1);
			CSC(cudaGetLastError());
		...
		if (kb + 1 < box[2]) {
			kernel_copy_send_DU<<<GRID_SIZE, BLOCK_SIZE>>>(dev_buff, dev_data, dim[0], dim[1], dim[2]-1);
			CSC(cudaGetLastError());
		...
		if (ib > 0) {
			kernel_copy_send_LR<<<GRID_SIZE, BLOCK_SIZE>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], 0);
			CSC(cudaGetLastError());
			CSC(cudaMemcpy(buff, dev_buff, sizeof(double) * dim[1] * dim[2], cudaMemcpyDeviceToHost));
			MPI_Bsend(buff, dim[1] * dim[2], MPI_DOUBLE, _ib(ib - 1, jb, kb), id, MPI_COMM_WORLD);
		}
		...
		if (jb > 0) {
			kernel_copy_send_FB<<<GRID_SIZE, BLOCK_SIZE>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], 0);
			CSC(cudaGetLastError());
			CSC(cudaMemcpy(buff, dev_buff, sizeof(double) * dim[0] * dim[2], cudaMemcpyDeviceToHost));
			MPI_Bsend(buff, dim[0] * dim[2], MPI_DOUBLE, _ib(ib, jb - 1, kb), id, MPI_COMM_WORLD);
		}
		...
		if (kb > 0) {
			kernel_copy_send_DU<<<GRID_SIZE, BLOCK_SIZE>>>(dev_buff, dev_data, dim[0], dim[1], 0);
			CSC(cudaGetLastError());
			CSC(cudaMemcpy(buff, dev_buff, sizeof(double) * dim[0] * dim[1], cudaMemcpyDeviceToHost));
			MPI_Bsend(buff, dim[0] * dim[1], MPI_DOUBLE, _ib(ib, jb, kb - 1), id, MPI_COMM_WORLD);
		}
		...
		if (ib > 0) {
			...
			kernel_copy_recive_LR<<<GRID_SIZE, BLOCK_SIZE>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], -1);
			CSC(cudaGetLastError());
		} else {
			kernel_LR_bc<<<GRID_SIZE, BLOCK_SIZE>>>(dev_data, dim[0], dim[1], dim[2], u[0], -1);
			CSC(cudaGetLastError());
		}
		...
		if (jb > 0) {
			...
			kernel_copy_recive_FB<<<GRID_SIZE, BLOCK_SIZE>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], -1);
			CSC(cudaGetLastError());
		} else {
			kernel_FB_bc<<<GRID_SIZE, BLOCK_SIZE>>>(dev_data, dim[0], dim[1], dim[2], u[2], -1);
			CSC(cudaGetLastError());
		}
		...
		if (kb > 0) {
			...
			kernel_copy_recive_DU<<<GRID_SIZE, BLOCK_SIZE>>>(dev_buff, dev_data, dim[0], dim[1], -1);
			CSC(cudaGetLastError());
		} else {
			kernel_DU_bc<<<GRID_SIZE, BLOCK_SIZE>>>(dev_data, dim[0], dim[1], u[4], -1);
			CSC(cudaGetLastError());
		}
		...
		if (ib + 1 < box[0]) {
			MPI_Recv(buff, dim[1] * dim[2], MPI_DOUBLE, _ib(ib + 1, jb, kb), _ib(ib + 1, jb, kb), MPI_COMM_WORLD, &status);
			CSC(cudaMemcpy(dev_buff, buff, sizeof(double) * dim[1] * dim[2], cudaMemcpyHostToDevice));
			kernel_copy_recive_LR<<<GRID_SIZE, BLOCK_SIZE>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], dim[0]);
			CSC(cudaGetLastError());
		} else {
			kernel_LR_bc<<<GRID_SIZE, BLOCK_SIZE>>>(dev_data, dim[0], dim[1], dim[2], u[1], dim[0]);
			CSC(cudaGetLastError());
		}
		...
		if (jb + 1 < box[1]) {
			MPI_Recv(buff, dim[0] * dim[2], MPI_DOUBLE, _ib(ib, jb + 1, kb), _ib(ib, jb + 1, kb), MPI_COMM_WORLD, &status);
			CSC(cudaMemcpy(dev_buff, buff, sizeof(double) * dim[0] * dim[2], cudaMemcpyHostToDevice));
			kernel_copy_recive_FB<<<GRID_SIZE, BLOCK_SIZE>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], dim[1]);
			CSC(cudaGetLastError());
		} else {
			kernel_FB_bc<<<GRID_SIZE, BLOCK_SIZE>>>(dev_data, dim[0], dim[1], dim[2], u[3], dim[1]);
			CSC(cudaGetLastError());
		}
		...
		if (kb + 1 < box[2]) {
			MPI_Recv(buff, dim[0] * dim[1], MPI_DOUBLE, _ib(ib, jb, kb + 1), _ib(ib, jb, kb + 1), MPI_COMM_WORLD, &status);
			CSC(cudaMemcpy(dev_buff, buff, sizeof(double) * dim[0] * dim[1], cudaMemcpyHostToDevice));
			kernel_copy_recive_DU<<<GRID_SIZE, BLOCK_SIZE>>>(dev_buff, dev_data, dim[0], dim[1], dim[2]);
			CSC(cudaGetLastError());
		} else {
			kernel_DU_bc<<<GRID_SIZE, BLOCK_SIZE>>>(dev_data, dim[0], dim[1], u[5], dim[2]);
			CSC(cudaGetLastError());
		}
