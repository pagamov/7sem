...
int main(int argc, char * argv[]) {
	...
	while (f) {
		...
		if (ib + 1 < box[0]) {
			kernel_copy_send_LR<<<dim3(32,32), dim3(32,32)>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], dim[0]-1);
			CSC(cudaGetLastError());
		...
		if (jb + 1 < box[1]) {
			kernel_copy_send_FB<<<dim3(32,32), dim3(32,32)>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], dim[1]-1);
			CSC(cudaGetLastError());
		...
		if (kb + 1 < box[2]) {
			kernel_copy_send_DU<<<dim3(32,32), dim3(32,32)>>>(dev_buff, dev_data, dim[0], dim[1], dim[2]-1);
			CSC(cudaGetLastError());
		...
		if (ib > 0) {
			kernel_copy_send_LR<<<dim3(32,32), dim3(32,32)>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], 0);
			CSC(cudaGetLastError());
			CSC(cudaMemcpy(buff, dev_buff, sizeof(double) * dim[1] * dim[2], cudaMemcpyDeviceToHost));
			MPI_Bsend(buff, dim[1] * dim[2], MPI_DOUBLE, _ib(ib - 1, jb, kb), id, MPI_COMM_WORLD);
		}
		...
		if (jb > 0) {
			kernel_copy_send_FB<<<dim3(32,32), dim3(32,32)>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], 0);
			CSC(cudaGetLastError());
			CSC(cudaMemcpy(buff, dev_buff, sizeof(double) * dim[0] * dim[2], cudaMemcpyDeviceToHost));
			MPI_Bsend(buff, dim[0] * dim[2], MPI_DOUBLE, _ib(ib, jb - 1, kb), id, MPI_COMM_WORLD);
		}
		...
		if (kb > 0) {
			kernel_copy_send_DU<<<dim3(32,32), dim3(32,32)>>>(dev_buff, dev_data, dim[0], dim[1], 0);
			CSC(cudaGetLastError());
			CSC(cudaMemcpy(buff, dev_buff, sizeof(double) * dim[0] * dim[1], cudaMemcpyDeviceToHost));
			MPI_Bsend(buff, dim[0] * dim[1], MPI_DOUBLE, _ib(ib, jb, kb - 1), id, MPI_COMM_WORLD);
		}
		...
		if (ib > 0) {
			...
			kernel_copy_recive_LR<<<dim3(32,32), dim3(32,32)>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], -1);
			CSC(cudaGetLastError());
		} else {
			kernel_LR_bc<<<dim3(32,32), dim3(32,32)>>>(dev_data, dim[0], dim[1], dim[2], left, -1);
			CSC(cudaGetLastError());
		}
		...
		if (jb > 0) {
			...
			kernel_copy_recive_FB<<<dim3(32,32), dim3(32,32)>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], -1);
			CSC(cudaGetLastError());
		} else {
			kernel_FB_bc<<<dim3(32,32), dim3(32,32)>>>(dev_data, dim[0], dim[1], dim[2], front, -1);
			CSC(cudaGetLastError());
		}
		...
		if (kb > 0) {
			...
			kernel_copy_recive_DU<<<dim3(32,32), dim3(32,32)>>>(dev_buff, dev_data, dim[0], dim[1], -1);
			CSC(cudaGetLastError());
		} else {
			kernel_DU_bc<<<dim3(32,32), dim3(32,32)>>>(dev_data, dim[0], dim[1], down, -1);
			CSC(cudaGetLastError());
		}
		...
		if (ib + 1 < box[0]) {
			MPI_Recv(buff, dim[1] * dim[2], MPI_DOUBLE, _ib(ib + 1, jb, kb), _ib(ib + 1, jb, kb), MPI_COMM_WORLD, &status);
			CSC(cudaMemcpy(dev_buff, buff, sizeof(double) * dim[1] * dim[2], cudaMemcpyHostToDevice));
			kernel_copy_recive_LR<<<dim3(32,32), dim3(32,32)>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], dim[0]);
			CSC(cudaGetLastError());
		} else {
			kernel_LR_bc<<<dim3(32,32), dim3(32,32)>>>(dev_data, dim[0], dim[1], dim[2], right, dim[0]);
			CSC(cudaGetLastError());
		}
		...
		if (jb + 1 < box[1]) {
			MPI_Recv(buff, dim[0] * dim[2], MPI_DOUBLE, _ib(ib, jb + 1, kb), _ib(ib, jb + 1, kb), MPI_COMM_WORLD, &status);
			CSC(cudaMemcpy(dev_buff, buff, sizeof(double) * dim[0] * dim[2], cudaMemcpyHostToDevice));
			kernel_copy_recive_FB<<<dim3(32,32), dim3(32,32)>>>(dev_buff, dev_data, dim[0], dim[1], dim[2], dim[1]);
			CSC(cudaGetLastError());
		} else {
			kernel_FB_bc<<<dim3(32,32), dim3(32,32)>>>(dev_data, dim[0], dim[1], dim[2], back, dim[1]);
			CSC(cudaGetLastError());
		}
		...
		if (kb + 1 < box[2]) {
			MPI_Recv(buff, dim[0] * dim[1], MPI_DOUBLE, _ib(ib, jb, kb + 1), _ib(ib, jb, kb + 1), MPI_COMM_WORLD, &status);
			CSC(cudaMemcpy(dev_buff, buff, sizeof(double) * dim[0] * dim[1], cudaMemcpyHostToDevice));
			kernel_copy_recive_DU<<<dim3(32,32), dim3(32,32)>>>(dev_buff, dev_data, dim[0], dim[1], dim[2]);
			CSC(cudaGetLastError());
		} else {
			kernel_DU_bc<<<dim3(32,32), dim3(32,32)>>>(dev_data, dim[0], dim[1], up, dim[2]);
			CSC(cudaGetLastError());
		}
		...
		kernel_get_vals<<<dim3(32, 32, 32), dim3(32, 4, 4)>>>(dev_data, dev_next, dim[0], dim[1], dim[2], hx, hy, hz);
		CSC(cudaGetLastError());

		kernel_get_diffs<<<dim3(32, 32, 32), dim3(32, 4, 4)>>>(dev_data, dev_next, dim[0], dim[1], dim[2]);
		CSC(cudaGetLastError());
	...
	int n_size = 14;
	char * bf = (char*)malloc(sizeof(char) * dim[0] * dim[1] * dim[2] * n_size);
	memset(bf, ' ', sizeof(char) * dim[0] * dim[1] * dim[2] * n_size);

	for (int k = 0; k < dim[2]; k++)
		for (int j = 0; j < dim[1]; j++)
			for (int i = 0; i < dim[0]; i++)
				sprintf(bf + (k * dim[0] * dim[1] + j * dim[0] + i) * n_size, "%.6e", data[_iD(i, j, k)]);

	for (int i = 0; i < dim[0] * dim[1] * dim[2] * n_size; i++)
		if (bf[i] == '\0')
			bf[i] = ' ';

	MPI_File fp;
	MPI_Datatype filetype;
	int sizes[] = {dim[2] * box[2], dim[1] * box[1], dim[0] * box[0] * n_size};
	int subsizes[] = {dim[2], dim[1], dim[0] * n_size};
	int starts[] = {id / (box[0] * box[1]) * dim[2], id % (box[0] * box[1]) / box[0] * dim[1],  id % (box[0] * box[1]) % box[0] * dim[0] * n_size};
	...
	MPI_File_write_all(fp, bf, dim[0] * dim[1] * dim[2] * n_size, MPI_CHAR, MPI_STATUS_IGNORE);
	MPI_File_close(&fp);
